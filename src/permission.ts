 ...
 */
const CHECK = <S = PermissionStatus>(result: S): boolean => result === RESULTS.GRANTED;
 ...
 * @returns {Promise<boolean>} - It returns a boolean value.
 */
 ...
 */
export const getStepCounterPermission = async (): Promise<boolean> => {
 ...
 */
export async function getBodySensorPermission(): Promise<boolean> {
  await openSettings();
